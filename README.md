...
- `src/config/site.ts` drives all search and AI metadata: `site.person` (name, alternate names, role, schools) and `site.keywords` (topics, used in the `Person.knowsAbout` JSON-LD, `<meta name="keywords">` and `/llms.txt`).
- The build emits a JSON-LD `@graph` on every page, Open Graph and Twitter tags, a sitemap with `lastmod`, `/llms.txt` and `/llms-full.txt`, and a `robots.txt` that allows search and AI crawlers. Tag pages and the 404 are `noindex`.
